...
template <>
void ConvergenceEvaluatorJans<Float>::evaluate()
{
    ...
    for (int i = 0; i < techniques.size(); ++i)
    {
        ...
        while (intervals->getNextInterval(val, samples)) // this is unecessary samples will always be 1
        {
            ...
            {
                evals[j] = techniques[i]->eval_jans_problem_secondary(problem, samples, cost, rng);
            }
            ...
            data_point.rmse = std::sqrt(data_point.sample_mse);
            ...
            // data_point.work_norm_var = avg_cost * data_point.true_mse;
            data_point.work_norm_var = avg_cost * data_point.sample_mse;
