...
template <>
void ConvergenceEvaluator<Float>::evaluate()
{
    ...
    for (int i = 0; i < techniques.size(); ++i)
    {
        ...
        while (intervals->getNextInterval(val, samples))
        {
            ...
            {
                evals[j] = techniques[i]->eval_problem_secondary(problem, samples, cost, rng);
            }

            Float true_rmse = 0.0;
            Float sample_rmse = 0.0;
            Float sample_mean = 0.0;
            Float avg_cost = cost / Float(trials * samples);
