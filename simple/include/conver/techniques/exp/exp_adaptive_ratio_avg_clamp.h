...
template <typename T>
struct AdaptiveRatioAvgClampExp : Technique<T>
{
    AdaptiveRatioAvgClampExp(Float maj) : maj(maj)
    {
        // blah
    }
        ...
        Float start_maj)
    {
        ...
        while (true)
        {
            ...
            Float tmp_maj = std::max(current_maj - dens, maj * 0.1);
            // update the null majorant
            step_maj = step_maj * (Float(steps) / Float(1 + steps)) + tmp_maj * 1.0 / (Float(1 + steps));
    ...
    virtual std::string getName() const { return "adaptive_ratio_avg_clamp"; }
