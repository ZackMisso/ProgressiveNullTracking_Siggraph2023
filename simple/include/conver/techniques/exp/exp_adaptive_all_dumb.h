...
template <typename T>
struct AdaptiveAllDumbExp : Technique<T>
{
    AdaptiveAllDumbExp(Float maj) : maj(maj)
    {
        // blah
    }
    ...
    virtual T eval_problem(const Problem<T> *problem, int samples, int &cost, pcg32 &rng)
    {
        ...
        t = problem->getBounds()->getEnd();

        Float dens = problem->eval_function(t);
        Float faux_maj = dens;
        Float dist = problem->getBounds()->getEnd() - problem->getBounds()->getStart();

        return exp(dist * (maj_to_use - dens));
        // {
        //     last_t = t;
    ...
    virtual std::string getName() const { return "adaptive_all_dumb_exp"; }
