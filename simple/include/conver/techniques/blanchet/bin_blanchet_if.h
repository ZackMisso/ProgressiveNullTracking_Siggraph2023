...
template <typename T>
struct BlanchetBinIf : Technique<T>
{
    BlanchetBinIf(Float r = 0.65) : blanchet_r(r) {}
    ...
    virtual T eval_problem(const Problem<T> *problem, int samples, int &cost, pcg32 &rng)
    {
        ...
        for (int i = 0; i < total_n; ++i)
        {
            ...
            base += problem->eval_rep_if_zero(problem->eval_integral(loc), min_val);
        ...
        base /= Float(total_n);
        Float p_1 = problem->eval_rep_if_zero(problem->eval_integral(p_1_samples), min_val / pow(2, n));
        Float p_2 = problem->eval_rep_if_zero(problem->eval_integral(p_2_samples), min_val / pow(2, n));
        Float p1 = problem->eval_rep_if_zero(problem->eval_integral(p1_samples), min_val / pow(2, n + 1));

        // return the result
        return base + (p1 - 0.5 * (p_1 + p_2)) / blanchet_pmf;
    }

    virtual std::string getName() const { return "blanchet_bin_if_" + std::to_string(int(blanchet_r * 100.0)); }
