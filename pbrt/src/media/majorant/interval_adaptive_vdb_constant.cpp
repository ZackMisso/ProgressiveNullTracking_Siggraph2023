...
// this datastructure is utilized by figure_7 to hardcode what the majorant is
// for all voxels to the same value. we still utilize this datastructure to take
// advantage of empty space skipping to save some compute time rather than just
// use a constant majorant.

IntervalAdaptiveDensityVDBConstant::IntervalAdaptiveDensityVDBConstant(
    const DensityFunction* density, Float global_maj, int dim)
    : MajorantFunction(density), global_maj(global_maj), dim(dim) {
    // std::cout << "GLOBAL MAJ: " << global_maj << std::endl;
    initialize(density);
}
...
IntervalAdaptiveDensityVDBConstant::MacroVoxelTraversal(const Point3f& pa,
                                                        const Point3f& pb,
                                                        Float baseTMin,
...
Float IntervalAdaptiveDensityVDBConstant::Maj(const Point3f& p) const {
...
void IntervalAdaptiveDensityVDBConstant::debug(Point3f p) const {
    int x = floor((p[0] - (-0.5)) * Float(dim));
    int y = floor((p[1] - (-0.5)) * Float(dim));
    int z = floor((p[2] - (-0.5)) * Float(dim));
...
Float IntervalAdaptiveDensityVDBConstant::Maj(const Point3f& pa,
...
void IntervalAdaptiveDensityVDBConstant::setMajVoxel(int x, int y, int z,
                                                     Float maj) {
    if (x < 0) x = 0;
    if (y < 0) y = 0;
    if (z < 0) z = 0;
    ...
    majGrid[z * dim * dim + y * dim + x] = global_maj;
...
Float IntervalAdaptiveDensityVDBConstant::getMajVoxel(int x, int y,
                                                      int z) const {
    if (x < 0) x = 0;
    if (y < 0) y = 0;
    if (z < 0) z = 0;
...
void IntervalAdaptiveDensityVDBConstant::initialize(
...
void IntervalAdaptiveDensityVDBConstant::UpdateMajorant(const Point3f& p,
                                                        Float density) {
...
void IntervalAdaptiveDensityVDBConstant::RenderVerticalSlice(
