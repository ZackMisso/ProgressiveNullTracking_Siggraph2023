...
IntervalAdaptiveDensityVDB::IntervalAdaptiveDensityVDB(
...
IntervalAdaptiveDensityVDB::~IntervalAdaptiveDensityVDB() { delete[] majGrid; }

std::vector<MacroGridQuery> IntervalAdaptiveDensityVDB::MacroVoxelTraversal(
...
Float IntervalAdaptiveDensityVDB::Maj(const Point3f& p) const {
...
void IntervalAdaptiveDensityVDB::debug(Point3f p) const {
    int x = floor((p[0] - (-0.5)) * Float(dim));
    int y = floor((p[1] - (-0.5)) * Float(dim));
    int z = floor((p[2] - (-0.5)) * Float(dim));
...
Float IntervalAdaptiveDensityVDB::Maj(const Point3f& pa,
...
void IntervalAdaptiveDensityVDB::setMajVoxel(int x, int y, int z, Float maj) {
    // std::cout << "PRE x: " << x << " y: " << y << " z: " << z << std::endl;
...
Float IntervalAdaptiveDensityVDB::getMajVoxel(int x, int y, int z) const {
    if (x < 0) x = 0;
    if (y < 0) y = 0;
    if (z < 0) z = 0;
    if (x >= dim) x = dim - 1;
...
void IntervalAdaptiveDensityVDB::initialize(const DensityFunction* dense) {
    ...
    // Vector3f density_cross = Vector3f(density_max[0] - density_min[0],
    //                                   density_max[1] - density_min[1],
    //                                   density_max[2] - density_min[2]);
    ...
    // std::cout << "length: " << density_cross[0] << ", " << density_cross[1]
    //           << ", " << density_cross[2] << std::endl;

    // std::cout << "box width: " << box_width[0] << ", " << box_width[2] << ",
    // "
    //           << box_width[1] << std::endl;
    ...
    for (int x = 0; x < dim; ++x) {
        for (int y = 0; y < dim; ++y) {
            for (int z = 0; z < dim; ++z) {
                ...
                // std::cout << "VOXEL CENTER: " << voxel_center[0] << ", "
                //           << voxel_center[2] << ", " << voxel_center[1]
                //           << std::endl;

                Vector3f min_exact = voxel_center - box_width;
                Vector3f max_exact = voxel_center + box_width;

                // std::cout << "MIN EXACT: " << min_exact[0] << ", "
                //           << min_exact[1] << ", " << min_exact[2] <<
                //           std::endl;

                // std::cout << "MAX EXACT: " << max_exact[0] << ", "
                //           << max_exact[1] << ", " << max_exact[2] <<
                //           std::endl;
                ...
                // std::cout << "MIN: " << min[0] << ", " << min[1] << ", "
                //           << min[2] << std::endl;

                // std::cout << "MAX: " << max[0] << ", " << max[1] << ", "
                //           << max[2] << std::endl;
                ...
                // setMajVoxel(
                //     x, y, z,
                //     std::min(dense->getMaxDensityInCube(min, max)[0], 0.35));
            }
        }
    }
...
void IntervalAdaptiveDensityVDB::UpdateMajorant(const Point3f& p,
                                                Float density) {
    int x = floor((p[0] - (-0.5)) * Float(dim));
    int y = floor((p[1] - (-0.5)) * Float(dim));
    int z = floor((p[2] - (-0.5)) * Float(dim));
...
void IntervalAdaptiveDensityVDB::RenderVerticalSlice(std::string filename,
                                                     int ps) const {
    if (ps != 1) return;
