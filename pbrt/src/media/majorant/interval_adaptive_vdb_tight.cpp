...
IntervalAdaptiveDensityVDBTight::IntervalAdaptiveDensityVDBTight(
...
IntervalAdaptiveDensityVDBTight::MacroVoxelTraversal(const Point3f& pa,
                                                     const Point3f& pb,
                                                     Float baseTMin,
...
Float IntervalAdaptiveDensityVDBTight::Maj(const Point3f& p) const {
...
void IntervalAdaptiveDensityVDBTight::debug(Point3f p) const {
    int x = floor((p[0] - (-0.5)) * Float(dim));
    int y = floor((p[1] - (-0.5)) * Float(dim));
    int z = floor((p[2] - (-0.5)) * Float(dim));
...
Float IntervalAdaptiveDensityVDBTight::Maj(const Point3f& pa,
...
void IntervalAdaptiveDensityVDBTight::setMajVoxel(int x, int y, int z,
                                                  Float maj) {
    if (x < 0) x = 0;
    if (y < 0) y = 0;
    if (z < 0) z = 0;
...
void IntervalAdaptiveDensityVDBTight::setGTMajVoxel(int x, int y, int z,
                                                    Float maj) {
    if (x < 0) x = 0;
    if (y < 0) y = 0;
    if (z < 0) z = 0;
...
Float IntervalAdaptiveDensityVDBTight::getMajVoxel(int x, int y, int z) const {
    if (x < 0) x = 0;
    if (y < 0) y = 0;
    if (z < 0) z = 0;
    if (x >= dim) x = dim - 1;
...
Float IntervalAdaptiveDensityVDBTight::getGTMajVoxel(int x, int y,
                                                     int z) const {
    if (x < 0) x = 0;
    if (y < 0) y = 0;
    if (z < 0) z = 0;
...
void IntervalAdaptiveDensityVDBTight::initialize(const DensityFunction* dense) {
...
void IntervalAdaptiveDensityVDBTight::UpdateMajorant(const Point3f& p,
                                                     Float density) {
    int x = floor((p[0] - (-0.5)) * Float(dim));
    int y = floor((p[1] - (-0.5)) * Float(dim));
    int z = floor((p[2] - (-0.5)) * Float(dim));
...
void IntervalAdaptiveDensityVDBTight::RenderVerticalSlice(std::string filename,
                                                          int ps) const {
