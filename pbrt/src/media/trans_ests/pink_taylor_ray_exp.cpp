...
int PinkTaylorRayExp::determine_tuple_size(Float maj_thik, Float p_z, Float c) const {
...
int PinkTaylorRayExp::ncmf(Float maj_thik) const {
    return ceil(pow((0.015 + maj_thik) * (0.65 + maj_thik) * (60.3 + maj_thik),
                    1.0 / 3.0));
}

void PinkTaylorRayExp::aggressive_russian_roulette(Sampler &sampler, Float p_z,
...
void PinkTaylorRayExp::pivot_array(const std::vector<Spectrum> &samples,
...
void PinkTaylorRayExp::elementry_means(const std::vector<Spectrum> &samples,
...
Float PinkTaylorRayExp::expected_order(Float c, Float p_z) const {
    Float K = floor(c);
    Float t = 1.0;
    Float sum = 1.0;
...
Spectrum PinkTaylorRayExp::Tr_ManualSig(const Ray &ray, Sampler &sampler, Float tMin,
...
Spectrum PinkTaylorRayExp::Tr(const Ray& ray, Sampler& sampler, Float tMin,
    ...
    int m = determine_tuple_size(maj * l * mult, p_z, c);
    ...
    for (int i = 0; i <= k; ++i) {
        ...
        for (int j = 0; j < m; ++j) {
            Float loc = l / Float(m) * (u_i + j);
            x_i += density->D(ray(path.unmap(loc))) * sigma_t * mult;
        }
    ...
    if (startOnSurface && !endOnSurface) return trans * mult;
    if (!startOnSurface && endOnSurface) return trans * mult;
    if (!startOnSurface && !endOnSurface) return trans * mult * mult;
