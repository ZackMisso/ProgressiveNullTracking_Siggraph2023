...
void VDB_DensityFunction::preProcess() {
    ...
    std::cout << "xTransFactor final: " << xTransFactor << std::endl;
    std::cout << "yTransFactor final: " << yTransFactor << std::endl;
    std::cout << "zTransFactor final: " << zTransFactor << std::endl;
}
...
Spectrum VDB_DensityFunction::D(const Point3f& p, bool buildState) const {
    ...
    if (worldSpace) {
        // Note: houdini stores the up axis as y while most pbrt scens have z as
        // the up vector
        //       so the indices are swivled.

        // index space sample
        // Float indexValue = (Float)sampler.isSample(openvdb::Vec3R(p[0], p[1],
        // p[2]));
        std::cout << "WHOOP" << std::endl;
        ...
        //     int((p[0] - box_min_x) * xScaleFactor - xTransFactor + min.x()),
        //     int((p[2] - box_min_y) * yScaleFactor - yTransFactor + min.y()),
        //     int((p[1] - box_min_z) * zScaleFactor - zTransFactor + min.z()));

        Point3i coord = Point3i(
            floor((p[0] - box_min_x) * xScaleFactor - xTransFactor + min.x()),
            floor((p[1] - box_min_y) * yScaleFactor - yTransFactor + min.y()),
            floor((p[2] - box_min_z) * zScaleFactor - zTransFactor + min.z()));
...
Spectrum VDB_DensityFunction::D_Debug(const Point3f& p, bool buildState) const {
    nDensityCalls++;
    ...
    // std::cout << "D_COORD BOX_MIN: " << box_min_x << ", " << box_min_y << ",
    // "
    //           << box_min_z << std::endl;

    // std::cout << "D_COORD SCALE: " << xScaleFactor << ", " << yScaleFactor
    //           << ", " << zScaleFactor << std::endl;

    // std::cout << "D_COORD PRE_SCALE: " << (p[0] - box_min_x) * xScaleFactor
    //           << ", " << (p[1] - box_min_y) * yScaleFactor << ", "
    //           << (p[2] - box_min_z) * zScaleFactor << std::endl;

    // std::cout << "D_COORD PRE_LOC: " << (p[0] - box_min_x) << ", "
    //           << (p[1] - box_min_x) << ", " << (p[2] - box_min_x) <<
    //           std::endl;

    // std::cout
    //     << "D_COORD IND: "
    //     << std::floor(Float(coord[0] - min.x()) / Float(max.x() - min.x()) *
    //     3)
    //     << ", "
    //     << std::floor(Float(coord[1] - min.y()) / Float(max.y() - min.y()) *
    //     3)
    //     << ", "
    //     << std::floor(Float(coord[2] - min.z()) / Float(max.z() - min.z()) *
    //     3)
    //     << std::endl;

    // std::cout << "D_COORD PREC: "
    //           << Float(coord[0] - min.x()) / Float(max.x() - min.x()) << ", "
    //           << Float(coord[1] - min.y()) / Float(max.y() - min.y()) << ", "
    //           << Float(coord[2] - min.z()) / Float(max.z() - min.z())
    //           << std::endl;

    // std::cout << "D_COORD LOC: " << p[0] << ", " << p[1] << ", " << p[2]
    //           << std::endl;
