...
class VDB_DensityFunction : public DensityFunction {
  public:
    ...
    virtual Vector3f getDimensionLengths() const {
        return Vector3f(xScaleFactor, yScaleFactor, zScaleFactor);
    }
    virtual Vector3f getTransFactor() const {
        return Vector3f(xTransFactor, yTransFactor, zTransFactor);
    }
  ...
  protected:
    openvdb::GridBase::Ptr
        baseGrid;  // this is here to correctly clear memory after rendering in
                   // case multiple renders are ran through pbrt api calls
    openvdb::FloatGrid::Ptr grid;

    // meta-settings
    bool worldSpace;      // if this density function should be applied in world
                          // space
