...
Spectrum FF_Trans_Weighted_Sampler::Sample_ManualSigs(
...
Spectrum FF_Trans_Weighted_Sampler::Sample_MutableSigs(
    const Ray& ray, const Ray& rWorld, Sampler& sampler, MemoryArena& arena,
    MediumInteraction* mi, Float tMin, Float tMax, Float sig_t, Float sig_s,
    bool& success) {
    sampler.BeforeFreeFlightSample();
    ...
    //     if (dense * inv > sampler.Get1D()) {
    //         // Populate _mi_ with medium interaction information and
    //         return weight *= (dense * invMaxDensity) / (dense * inv) *
    //         (sig_s / sig_t); PhaseFunction* phase = ARENA_ALLOC(arena,
    ...
    //     weight *= (diff * invMaxDensity) / (std::abs(diff) * inv);
    // }

    // return Spectrum(weight);
}

Spectrum FF_Trans_Weighted_Sampler::Sample_Cull(
    const Ray& ray, const Ray& rWorld, Sampler& sampler, MemoryArena& arena,
    MediumInteraction* mi, Float tMin, Float tMax, Float sig_t, Float sig_s,
    bool& success) const {
    sampler.BeforeFreeFlightSample();
